...
chmod +x ops/oci/backup_postgres.sh ops/oci/watchdog_micro.sh ops/oci/write_host_metrics.sh ops/oci/check_host_ports.sh ops/oci/dr_check.sh ops/oci/try_create_ampere_a1.sh
chmod +x ops/recovery/backup_postgres.sh ops/recovery/verify_restore.sh ops/recovery/dr_check.sh
...
crontab -l 2>/dev/null \
  ...
  | grep -v 'sentinela/ops/oci/test_restore_backup.sh' \
...
47 3 * * 0 /home/ubuntu/sentinela/ops/oci/test_restore_backup.sh >> /home/ubuntu/sentinela/ops/oci/restore.log 2>&1
CRON
crontab "$tmp_cron"
rm -f "$tmp_cron"
